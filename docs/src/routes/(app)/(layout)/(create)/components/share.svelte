...
<Popover.Root>
	<Popover.Trigger class={cn(buttonVariants({ variant: "outline", size: "sm" }), "hidden md:flex")}>
		<ShareIcon />
		Share
	</Popover.Trigger>
	<Popover.Content align="end" class="w-96">
		<div class="flex w-full flex-col items-start gap-2">
			<div class="h-[183px] w-full overflow-hidden rounded-lg border border-border bg-background">
			...
			<div class="flex w-full place-items-center items-center gap-2">
				...
				<Button variant="outline" size="icon" onclick={() => clipboard.copy(designSystem.shareUrl)}>
					{#if clipboard.copied}
