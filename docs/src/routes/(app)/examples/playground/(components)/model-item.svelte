...
<Command.Item value={model.name} {onSelect} {...restProps}>
	{#snippet child({ props })}
		<div
			...
			{...props}
			class="aria-selected:bg-primary aria-selected:text-primary-foreground data-disabled:pointer-events-none data-disabled:opacity-50 relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none"
		>
			{model.name}
			{#if isSelected}
