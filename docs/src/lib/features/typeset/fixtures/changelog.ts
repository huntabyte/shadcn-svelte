...
<ul>
  ...
  <li><strong>Changed:</strong> selectors are memoized per subscriber, cutting reactive update counts roughly in half on wide stores.</li>
  <li><strong>Fixed:</strong> subscribing during a notification no longer skips the next update.</li>
  <li><strong>Fixed:</strong> <code>equals</code> is respected for the initial store subscription, matching the documented behavior.</li>
</ul>
...
<ul>
  <li><strong>Fixed:</strong> a race where two synchronous writes in the same tick could notify in reverse order under Svelte’s batched updates.</li>
...
<ul>
  <li><strong>Added:</strong> Svelte Native support; store subscriptions no longer touch <code>window</code>.</li>
