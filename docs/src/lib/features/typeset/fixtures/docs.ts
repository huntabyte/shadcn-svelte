...
<p>The <code>Chat</code> class makes it effortless to create a conversational user interface for your chatbot application. It enables the streaming of chat messages from your AI provider, manages the chat state, and updates the UI automatically as new messages arrive.</p>
<p>To summarize, the <code>Chat</code> class provides the following features:</p>
<ul>
<li><strong>Message Streaming</strong>: All the messages from the AI provider are streamed to the chat UI in real-time.</li>
<li><strong>Managed States</strong>: The class manages the states for input, messages, status, error and more for you.</li>
<li><strong>Seamless Integration</strong>: Easily integrate your chat AI into any design or layout with minimal effort.</li>
</ul>
<p>In this guide, you will learn how to use the <code>Chat</code> class to create a chatbot application with real-time message streaming. Check out our <a href="/docs/ai-sdk-ui/chatbot-tool-usage">chatbot with tools guide</a> to learn how to use tools in your chatbot.</p>
<h2>Example</h2>
...
<li>The class appends chunks to the last message as they arrive, updating the UI as it goes.</li>
</ol>
...
<p>In the <code>Chat</code> component, the <code>Chat</code> class will request to your AI provider endpoint whenever the user sends a message using <code>sendMessage</code>. The messages are then streamed back in real-time and displayed in the chat UI.</p>
<h2>Customized UI</h2>
<p><code>Chat</code> also provides ways to manage the chat message states via code, show status, and update messages without being triggered by user interactions.</p>
<h3>Status</h3>
<p>The <code>Chat</code> class exposes a <code>status</code>. It has the following possible values:</p>
<ul>
...
{#if chat.status === 'submitted' || chat.status === 'streaming'}
  &lt;div&gt;
    {#if chat.status === 'submitted'}&lt;Spinner /&gt;{/if}
...
<p>It's also a common use case to abort the response message while it's still streaming back from the AI provider. You can do this by calling the <code>stop</code> method on the <code>Chat</code> instance.</p>
<pre><code>const chat = new Chat();
...
<p><code>Chat</code> provides optional event callbacks that you can use to handle different stages of the chatbot lifecycle:</p>
<ul>
