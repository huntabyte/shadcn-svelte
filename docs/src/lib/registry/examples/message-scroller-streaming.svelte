	...
	<div class="relative flex flex-col gap-4">
		<Card.Root class="mx-auto h-140 w-full max-w-sm gap-0">
			<Card.Header class="gap-1 border-b">
				...
				<Card.Description>Auto-scroll follows the live edge of the conversation.</Card.Description>
				<Card.Action>
					<Tooltip.Root>
						<Tooltip.Trigger>
							{#snippet child({ props })}
				...
				{#if demo.messages.length === 0}
					<Empty.Root class="h-full">
						<Empty.Header>
							...
							<Empty.Description>Press send to stream a scripted launch summary.</Empty.Description>
						</Empty.Header>
					</Empty.Root>
				{:else}
