...
type ChatNextOptions<METADATA, DATA_PARTS extends UIDataTypes, TOOLS extends UITools> = {
...
type ChatDataPart<DATA_PARTS extends UIDataTypes> = StreamDataPart<DATA_PARTS> & {
	order: number;
	delayMs: number;
};
...
type ChatTurn<METADATA, DATA_PARTS extends UIDataTypes, TOOLS extends UITools> = {
...
function cloneMessage<METADATA, DATA_PARTS extends UIDataTypes, TOOLS extends UITools>(
	message: UIMessage<METADATA, DATA_PARTS, TOOLS>
) {
	return {
		...message,
...
function createToolPart(name: string, toolCallId: string, options: ToolPartOptions = {}) {
	const state =
		options.state ??
		(options.errorText
			? "output-error"
...
function replaceOrPushPart<DATA_PARTS extends UIDataTypes, TOOLS extends UITools>(
...
function materializeEvents<DATA_PARTS extends UIDataTypes, TOOLS extends UITools>(
	events: ChatWriterEvent<DATA_PARTS, TOOLS>[]
...
>(options: AiMessageFakerOptions = {}) {
	...
	const parts = {
		...
		reasoningFile(options: Partial<ReasoningFileUIPart> = {}): ReasoningFileUIPart {
			return {
				type: "reasoning-file",
		...
		sourceDocument(options: Partial<SourceDocumentUIPart> = {}): SourceDocumentUIPart {
			return {
		...
		dataUpdates(
			name: keyof DATA_PARTS & string,
			updates: Array<DATA_PARTS[keyof DATA_PARTS & string]>,
		...
		tool(name: keyof TOOLS & string, options: ToolPartOptions = {}): ToolUIPart<TOOLS> {
		...
		dynamicTool(toolName: string, options: ToolPartOptions = {}): DynamicToolUIPart {
			return {
				...createToolPart(toolName, options.toolCallId ?? nextToolCallId(), options),
				type: "dynamic-tool",
				toolName,
	...
	function assistant(
		messageParts: Array<UIMessagePart<DATA_PARTS, TOOLS>> = [parts.text()] as Array<
	...
	function builder(
		...
		const api = {
			...
			tool(name: keyof TOOLS & string, options?: ToolPartOptions) {
	...
	function createWriter(events: ChatWriterEvent<DATA_PARTS, TOOLS>[]) {
		...
		const writer = {
			...
			reasoning(text = DEFAULT_REASONING_PART, options: StreamTextOptions = {}) {
				events.push({
					kind: "reasoning",
			...
			tool(name: keyof TOOLS & string, options: ToolWriterOptions = {}) {
	...
	function eventsFromParts(messageParts: Array<UIMessagePart<DATA_PARTS, TOOLS>>) {
	...
	async function enqueueTextEvent(
		...
		const deltas =
			event.options?.mode === "instant" ? [event.text] : splitTextDeltas(event.text);
	...
	function createResponseStream(
		...
		return new ReadableStream<InferUIMessageChunk<UIMessage<METADATA, DATA_PARTS, TOOLS>>>({
	...
	function chat() {
		...
		function findLatestScriptedIndex(messages: Array<UIMessage<METADATA, DATA_PARTS, TOOLS>>) {
		...
		function findNextUserTurn(messages: Array<UIMessage<METADATA, DATA_PARTS, TOOLS>>) {
			const latestScriptedIndex = findLatestScriptedIndex(messages);
		...
		const api = {
			...
			getData(options: ChatDataGetOptions = {}) {
				...
				return dataParts.filter((part) => part.order < count).map((part) => ({ ...part }));
			},
