...
description: Turn any public GitHub repository into a shadcn-svelte registry.
date: 2026-06-01
---
...
GitHub registries are source registries. You do not need to run
`shadcn-svelte registry build`, publish generated item JSON files, or set up a
registry server. The CLI reads the root `registry.json`, resolves `include`
entries, finds the requested item, and installs the files declared by that item.
...
GitHub registry addresses work with the registry commands supported by
`shadcn-svelte`.
