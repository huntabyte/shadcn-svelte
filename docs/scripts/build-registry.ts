...
const GENERATED_REGISTRY_PATH = path.resolve("src", "__registry__");
	...
	await Promise.all(
		PRESET_STYLES.map(async (style) => {
			...
			try {
				...
				// The CLI only creates/overwrites files in the output dir,
				// so we'll clean it first to prevent removed/renamed items from lingering.
				rimraf.sync(path.resolve(REGISTRY_PATH, "styles", style));
...
`;
	...
	// Write style index.
	const registryPath = path.resolve(GENERATED_REGISTRY_PATH, "index.js");
	...
	// Copy into a sibling temp dir and swap it in, so the mirror is never observed
	// half-written or missing by the (possibly running) Vite dev server.
	const jsonDir = path.resolve(GENERATED_REGISTRY_PATH, "json");
	const jsonTempDir = `${jsonDir}.temp`;
	rimraf.sync(jsonTempDir);
	fs.cpSync(REGISTRY_PATH, jsonTempDir, { recursive: true });
	rimraf.sync(jsonDir);
	fs.renameSync(jsonTempDir, jsonDir);
...
/**
 * Runs a one-off `build-registry` in a child process. Imported constants, such as `THEMES`, `fonts`
 * and the colors data, are static imports _in this module_, so re-running `build()` inside this long-lived
 * watcher process would keep using the values captured on the first run.
 */
function runBuildInChildProcess() {
	return new Promise<void>((resolve, reject) => {
		const child = spawn(
			process.execPath,
			[...process.execArgv, process.argv[1], "build-registry"],
			{ stdio: "inherit", cwd: process.cwd() }
		);
		child.on("error", reject);
		child.on("exit", (code, signal) => {
			if (code === 0) resolve();
			else reject(new Error(`registry build exited with ${signal ?? `code ${code}`}`));
		});
	});
}
