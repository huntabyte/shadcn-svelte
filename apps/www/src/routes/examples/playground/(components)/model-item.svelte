...
<Command.Item value={model.name} asChild {onSelect} let:action let:attrs>
	<div
		...
		use:action
		{...$$restProps}
		class="relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 aria-selected:bg-primary aria-selected:text-primary-foreground"
	>
		{model.name}
		{#if isSelected}
