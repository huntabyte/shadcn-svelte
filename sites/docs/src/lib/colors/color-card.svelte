<script lang="ts">
	...
	import { getColorsData } from "$lib/colors/colors.js";
...
<Card.Root>
	<div class="flex flex-col px-2 pt-5 md:px-5">
		{#each Object.entries(getColorsData()) as [colorName, shades] (colorName)}
			<div class="">
				...
				<div class="overflox-x-auto mb-5 flex gap-1 md:gap-2">
					{#each Object.entries(shades) as [shade, colorEntry] (shade)}
						<button
							...
							style="background-color: {(colorEntry as { hex: string }).hex};"
						>
