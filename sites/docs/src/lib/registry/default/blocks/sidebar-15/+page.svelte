<script lang="ts">
	import SidebarLeft from "$lib/registry/default/blocks/sidebar-15/components/sidebar-left.svelte";
	import SidebarRight from "$lib/registry/default/blocks/sidebar-15/components/sidebar-right.svelte";
	import * as Breadcrumb from "$lib/registry/default/ui/breadcrumb/index.js";
	import { Separator } from "$lib/registry/default/ui/separator/index.js";
	import * as Sidebar from "$lib/registry/default/ui/sidebar/index.js";
</script>

<Sidebar.Provider>
	<SidebarLeft />
	<Sidebar.Inset>
		<header class="bg-background sticky top-0 flex h-14 shrink-0 items-center gap-2">
			<div class="flex flex-1 items-center gap-2 px-3">
				<Sidebar.Trigger />
				<Separator orientation="vertical" class="mr-2 h-4" />
				<Breadcrumb.Root>
					<Breadcrumb.List>
						<Breadcrumb.Item>
							<Breadcrumb.Page class="line-clamp-1">
								Project Management & Task Tracking
							</Breadcrumb.Page>
						</Breadcrumb.Item>
					</Breadcrumb.List>
				</Breadcrumb.Root>
			</div>
		</header>
		<div class="flex flex-1 flex-col gap-4 p-4">
			<div class="bg-muted/50 mx-auto h-24 w-full max-w-3xl rounded-xl"></div>
			<div class="bg-muted/50 mx-auto h-[100vh] w-full max-w-3xl rounded-xl"></div>
		</div>
	</Sidebar.Inset>
	<SidebarRight />
</Sidebar.Provider>
