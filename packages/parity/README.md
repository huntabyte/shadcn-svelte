...
Calendar and range-calendar are both compared against upstream `calendar`, including all their split slot components. For other items, only files that have an upstream counterpart are compared. A file counts when its name matches the item, or when its PascalCase name matches a component exported by the upstream file, allowing for `Pane` ↔ `Panel` and `GroupHeading` ↔ `Label` aliases.
...
| `equivalent` | Differs only by tokens that are runtime-equivalent between Bits UI and Radix, such as `data-[state=open]` vs `data-open`, DayPicker boolean modifiers vs Bits presence attributes, DayPicker day buttons vs Bits day elements, physical vs logical sides (`pl-2` vs `ps-2`), or `aria-[orientation=…]` vs `data-horizontal`. Hidden unless `--verbose` or `--no-exclude-runtime-equivalent`. |
...
`chart` diverges too much from their Radix originals for a class-string comparison to be meaningful. It is skipped unless you pass `--include-skipped` or name one of them as `item`.

The calendar checks document the unsupported week-number column through source-level upstream ignores. Bits-only unavailable-date styling and RTL navigation utilities retain their own narrow ignores.
