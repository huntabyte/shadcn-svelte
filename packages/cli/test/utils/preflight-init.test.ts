...
describe("preflightInit", () => {
	...
	it("should throw error for Tailwind v3 + Svelte v5", () => {
		...
		expect(() => preflightInit("/test", { force: false })).toThrow(
	...
	it("should pass for Tailwind v4 + Svelte v5", () => {
		...
		expect(() => preflightInit("/test", { force: false })).not.toThrow();
	});
	...
	it("should continue with force flag", () => {
		vi.mocked(getProjectPackageInfo).mockReturnValue({
			dependencies: {},
			devDependencies: {},
		});

		expect(() => preflightInit("/test", { force: true })).not.toThrow();
	});
});
